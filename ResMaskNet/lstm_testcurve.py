...
class SimpleLSTM(nn.Module):
    def __init__(self, input_size=512, hidden_size=64, num_layers=1, dropout=0.4):
        super().__init__()
...
def train_one(train_loader, idx_train, scale, test_videos, subject_id,
    ...
    model = SimpleLSTM(input_size=X.shape[2]).to(device)
    ...
    writer = SummaryWriter(log_dir=f"runs/lstm_train_and_test_curves_overlap_/{subject_id}")
    ...
    for epoch in range(max_epochs):
        ...
        print(
            f"[{subject_id}] Epoch {epoch+1:03d} | "
...
# NEW: save video predictions for Excel
...
for test_subject in subjects:
    ...
    model = train_one(train_loader, idx_train, scale, test_videos, test_subject)
...
predictions_df.to_excel("emb_lstm_video_predictions.xlsx", index=False)

print("Saved video predictions to: emb_lstm_video_predictions.xlsx")
