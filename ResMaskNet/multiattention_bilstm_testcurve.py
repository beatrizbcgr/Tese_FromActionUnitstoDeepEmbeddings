...
class AttentionLSTM(nn.Module):
    def __init__(self, input_size=512, hidden_size=64, num_layers=1):
        ...
        self.attention = MultiHeadTemporalAttention(
            self.bi_hidden, num_heads=4  
        )

        self.hnorm = nn.LayerNorm(self.bi_hidden)
        self.fc = nn.Linear(self.bi_hidden, 1)
...
def train_one(train_loader, scale, test_videos, subject_id,
    ...
    # balancear dados- count positives and negatives in training set
    ...
    writer = SummaryWriter(
        log_dir=f"runs/multihead_attention_bilstm_balanced/{subject_id}"
    )
...
for test_subject in subjects:
    ...
    model = train_one(train_loader, scale, test_videos, test_subject)
    ...
    for v in test_videos:
        ...
        all_video_trues.append(video_to_label[v])
