...
# DATALOADER
# ============================================================
def create_loader(idx_train):
    scaler = StandardScaler().fit(X[idx_train].reshape(-1, X.shape[2]))

    def scale(idx):
        return torch.tensor(
            scaler.transform(X[idx].reshape(-1, X.shape[2])).reshape(X[idx].shape),
            dtype=torch.float32
        )
    ...
    return train_loader, scale

# ============================================================
# TRAIN FUNCTION
# ============================================================
def train_one(train_loader, idx_train, scale, test_videos, subject_id,
              max_epochs=100, patience=40):

    model = AttentionLSTM(input_size=X.shape[2]).to(device)
    ...
    # ============================================================
    n_pos = (y[idx_train] == 1).sum().item()
    n_neg = (y[idx_train] == 0).sum().item()
    ...
    writer = SummaryWriter(log_dir=f"runs/attention_lstm_trainandtestcurves_overlap_/{subject_id}")
    ...
    for epoch in range(max_epochs):
        ...
        print(f"[{subject_id}] Epoch {epoch+1:03d} | TrainLoss={train_loss:.4f} | TestAcc={test_acc:.4f}")
...
for test_subject in subjects:
    ...
    train_loader, scale = create_loader(idx_train)
    model = train_one(
    train_loader,
    idx_train,
    scale,
...
# ============================================================
predictions_df = pd.DataFrame(video_prediction_rows)
predictions_df.to_excel("attention_lstm_global_video_predictions.xlsx", index=False)

print("Saved video predictions to: attention_lstm_global_video_predictions.xlsx")

# ============================================================
# FINAL RESULTS
# ============================================================
print("\n===== FINAL VIDEO-LEVEL RESULTS =====")
print("Accuracy:", accuracy_score(all_video_trues, all_video_preds))
print("F1:", f1_score(all_video_trues, all_video_preds))
print("Confusion matrix:\n", confusion_matrix(all_video_trues, all_video_preds))