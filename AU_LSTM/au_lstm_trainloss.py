import numpy as np
import pandas as pd
import torch
import torch.nn as nn
from torch.utils.data import DataLoader, TensorDataset
from torch.utils.tensorboard import SummaryWriter
from sklearn.preprocessing import StandardScaler
from collections import defaultdict
import os
import random

# ============================================================
# LOAD DATA
# ============================================================

stroke_df = pd.read_csv("data/stroke_final.csv")
hc_df     = pd.read_csv("data/hc_final.csv")

df = pd.concat([stroke_df, hc_df], ignore_index=True)

# ============================================================
# REPRODUCIBILITY + DEVICE
# ============================================================

SEED = 42
random.seed(SEED)
np.random.seed(SEED)
torch.manual_seed(SEED)
torch.cuda.manual_seed_all(SEED)

torch.backends.cudnn.deterministic = True
torch.backends.cudnn.benchmark = False

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("Using device:", device)

# ============================================================
# TRAINING SETTINGS
# ============================================================

MAX_EPOCHS = 100
PATIENCE   = 50

# ============================================================
# SIMPLE LSTM
# ============================================================

class SimpleLSTM(nn.Module):
    def __init__(self, input_size, hidden_size=64, num_layers=1):
        super().__init__()
        self.norm = nn.LayerNorm(input_size)

        self.lstm = nn.LSTM(
            input_size=input_size,
            hidden_size=hidden_size,
            num_layers=num_layers,
            dropout=0.0,
            batch_first=True
        )

        self.hnorm = nn.LayerNorm(hidden_size)
        self.fc = nn.Linear(hidden_size, 1)

    def forward(self, x):
        x = self.norm(x)
        _, (hn, _) = self.lstm(x)
        h = hn[-1]
        h = self.hnorm(h)
        return self.fc(h).squeeze(-1)

# ============================================================
# TRAINING
# ============================================================

def train_model(train_loader, input_size, log_dir,
                scale, test_videos, video_to_chunks, video_to_label):

    model = SimpleLSTM(input_size=input_size).to(device)

    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)

    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, T_0=20, T_mult=1, eta_min=1e-5
    )

    criterion = nn.BCEWithLogitsLoss()

    writer = SummaryWriter(log_dir=log_dir)

    best_loss = float("inf")
    best_state = None
    patience_counter = 0

    for epoch in range(MAX_EPOCHS):

        # ---------------- TRAIN ----------------
        model.train()
        train_loss = 0.0

        for xb, yb in train_loader:
            xb, yb = xb.to(device), yb.to(device)

            optimizer.zero_grad()
            loss = criterion(model(xb), yb)
            loss.backward()
            optimizer.step()

            train_loss += loss.item() * len(yb)

        train_loss /= len(train_loader.dataset)

        scheduler.step(epoch)

        # ---------------- TEST ACC ----------------
        model.eval()

        preds = []
        trues = []

        with torch.no_grad():

            for v in test_videos:

                chunk_probs = []

                for idx in video_to_chunks[v]:

                    xb = scale([idx])
                    p = torch.sigmoid(model(xb.to(device))).item()
                    chunk_probs.append(p)

                pred = int(np.mean(chunk_probs) > 0.5)

                preds.append(pred)
                trues.append(video_to_label[v])

        test_acc = np.mean(np.array(preds) == np.array(trues))

        # ---------------- LOGGING ----------------
        writer.add_scalar("Loss/train", train_loss, epoch)
        writer.add_scalar("Accuracy/test", test_acc, epoch)
        writer.add_scalar("LR", optimizer.param_groups[0]["lr"], epoch)

        print(
            f"Epoch {epoch:03d} | "
            f"TrainLoss={train_loss:.4f} | "
            f"TestAcc={test_acc:.4f}"
        )

        # ---------------- MODEL SELECTION ----------------
        if train_loss < best_loss:
            best_loss = train_loss
            best_state = {k: v.cpu() for k, v in model.state_dict().items()}
            patience_counter = 0
        else:
            patience_counter += 1
            if patience_counter >= PATIENCE:
                break

    writer.close()

    model.load_state_dict(best_state)
    return model


# ============================================================
# DATA PREPARATION
# ============================================================

df["video"] = df["participant_id"].astype(str) + "_" + df["task"].astype(str)

au_cols = [c for c in df.columns if c.startswith("AU")]

video_to_seq, video_to_label, video_to_subject = {}, {}, {}

for v in df["video"].unique():

    sub = df[df["video"] == v].sort_values("frame")
    print(sub["frame"].head(20).values)

    video_to_seq[v] = sub[au_cols].values.astype(np.float32)
    video_to_label[v] = sub["label"].iloc[0]
    video_to_subject[v] = sub["participant_id"].iloc[0]

# ============================================================
# DEBUG: CHECK FRAME ORDER
# ============================================================

v = list(video_to_seq.keys())[0]
seq = video_to_seq[v]

frames = df[df["video"] == v].sort_values("frame")["frame"].values

print("Frames:", frames[:10])
print("First AU row:", seq[0])
print("Second AU row:", seq[1])

min_len = min(len(v) for v in video_to_seq.values())

chunks, labels = [], []
video_to_chunks = defaultdict(list)

OVERLAP = 0.30
stride = int(min_len * (1 - OVERLAP))

idx = 0

for v, seq in video_to_seq.items():

    start = 0
    L = len(seq)
    last_start = -1

    while True:

        end = start + min_len

        if end > L:

            end = L
            start = end - min_len

            if start <= last_start:
                break

            chunks.append(seq[start:end])
            labels.append(video_to_label[v])
            video_to_chunks[v].append(idx)
            idx += 1
            break

        chunks.append(seq[start:end])
        labels.append(video_to_label[v])
        video_to_chunks[v].append(idx)
        idx += 1

        last_start = start
        start += stride

X = torch.tensor(np.stack(chunks), dtype=torch.float32)
y = torch.tensor(labels, dtype=torch.float32)

subjects = df["participant_id"].unique()

# ============================================================
# GLOBAL LOSO
# ============================================================

all_preds = []
all_trues = []

# NEW: save chunk-level and video-level outputs for ensemble
chunk_preds_dict = {}
chunk_proba_dict = {}
video_preds_dict = {}
video_proba_dict = {}
true_dict = {}

for test_subject in subjects:

    print(f"\n=== TEST SUBJECT: {test_subject} ===")

    train_videos = [v for v in video_to_seq if video_to_subject[v] != test_subject]
    test_videos  = [v for v in video_to_seq if video_to_subject[v] == test_subject]

    idx_train = [i for v in train_videos for i in video_to_chunks[v]]

    scaler = StandardScaler().fit(X[idx_train].reshape(-1, X.shape[2]))

    def scale(idx):

        z = scaler.transform(X[idx].reshape(-1, X.shape[2]))

        return torch.tensor(
            z.reshape(len(idx), min_len, X.shape[2]),
            dtype=torch.float32
        )

    train_loader = DataLoader(
        TensorDataset(scale(idx_train), y[idx_train]),
        batch_size=32,
        shuffle=True
    )

    log_dir = f"runs/global_au_lstm_testcurve_overlap/{test_subject}"

    model = train_model(
        train_loader,
        len(au_cols),
        log_dir,
        scale,
        test_videos,
        video_to_chunks,
        video_to_label
    )

    # ---------------- FINAL TEST ----------------
    model.eval()

    for v in test_videos:

        idx_test = video_to_chunks[v]
        chunk_probs = []
        chunk_preds = []

        with torch.no_grad():

            for i_chunk in idx_test:

                xb = scale([i_chunk])
                p = torch.sigmoid(model(xb.to(device))).item()

                chunk_probs.append(float(p))
                chunk_preds.append(int(p > 0.5))

        pred = int(np.mean(chunk_probs) > 0.5)
        true = video_to_label[v]
        proba = float(np.mean(chunk_probs))

        # SAVE FOR ENSEMBLE
        chunk_preds_dict[v] = chunk_preds
        chunk_proba_dict[v] = chunk_probs
        video_preds_dict[v] = pred
        video_proba_dict[v] = proba
        true_dict[v] = true

        all_preds.append(pred)
        all_trues.append(true)

# ============================================================
# SAVE FOR ENSEMBLE
# ============================================================

np.save("aulstm_chunk_preds.npy", chunk_preds_dict)
np.save("aulstm_chunk_proba.npy", chunk_proba_dict)
np.save("aulstm_video_preds.npy", video_preds_dict)
np.save("aulstm_video_proba.npy", video_proba_dict)
np.save("true_labels.npy", true_dict)

print("Saved predictions for ensemble:")
print(" - aulstm_chunk_preds.npy")
print(" - aulstm_chunk_proba.npy")
print(" - aulstm_video_preds.npy")
print(" - aulstm_video_proba.npy")
print(" - true_labels.npy")

# ============================================================
# SAVE CSV / EXCEL RESULTS
# ============================================================

video_rows = []
chunk_rows = []

for v in video_preds_dict.keys():

    subject = video_to_subject[v]

    # task name from "participant_task"
    task = v.split("_", 1)[1]

    # ---------------- VIDEO LEVEL ----------------
    video_rows.append({
        "video": v,
        "subject": subject,
        "test_subject_fold": test_subject,
        "task": task,
        "true_label": true_dict[v],
        "video_prediction": video_preds_dict[v],
        "video_probability": video_proba_dict[v]
    })

    # ---------------- CHUNK LEVEL ----------------
    for c_idx, (pred, prob) in enumerate(
        zip(chunk_preds_dict[v], chunk_proba_dict[v])
    ):

        chunk_rows.append({
            "video": v,
            "subject": subject,
            "test_subject_fold": test_subject,
            "task": task,
            "chunk_id": c_idx,
            "true_label": true_dict[v],
            "chunk_prediction": pred,
            "chunk_probability": prob
        })

# create dfs
video_df = pd.DataFrame(video_rows)
chunk_df = pd.DataFrame(chunk_rows)

# save csv
video_df.to_csv("aulstm_video_predictions.csv", index=False)
chunk_df.to_csv("aulstm_chunk_predictions.csv", index=False)

# save excel
with pd.ExcelWriter("aulstm_predictions.xlsx") as writer:
    video_df.to_excel(writer, sheet_name="video_level", index=False)
    chunk_df.to_excel(writer, sheet_name="chunk_level", index=False)

print("\nSaved CSV/Excel prediction tables:")
print(" - aulstm_video_predictions.csv")
print(" - aulstm_chunk_predictions.csv")
print(" - aulstm_predictions.xlsx")

# ============================================================
# FINAL RESULTS
# ============================================================

all_preds = np.array(all_preds)
all_trues = np.array(all_trues)

acc = np.mean(all_preds == all_trues)

print("\n=== FINAL GLOBAL RESULTS ===")
print(f"Video-Accuracy: {acc:.3f}")